...
class BlanketOrder(models.Model):
    ...
    state = fields.Selection(
        selection=[
            ("draft", "Draft"),
            ("waiting_executive_manger_approval", "Waiting Executive Manger Approval"),
    ...
    quantity_type = fields.Selection(
        related='po_type_id.quantity_type'
    )
    need_project = fields.Boolean(
        related='po_type_id.need_project'
    )
    allow_validity_date = fields.Boolean(
        related='po_type_id.allow_validity_date'
    )
    project_id = fields.Many2one(
        'project'
    )
    start_date = fields.Date()
    all_original_quantity = fields.Float(
        compute='_compute_all_original_quantity',
        tracking=True
    )
    
    @api.depends('line_ids')
    def _compute_all_original_quantity(self):
        """ Compute all_original_quantity value """
        for rec in self:
            if rec.line_ids:
                rec.all_original_quantity = sum(rec.line_ids.mapped('original_uom_qty'))
            else:
                rec.all_original_quantity = 0
    ...
    def _onchange_project_id(self):
        ...
        for rec in self:
            if rec.project_id:
                project_lines = rec.project_id.project_line_ids.filtered(lambda x:x.partner_id == rec.partner_id)
                if project_lines:
                    rec.update({
                        'validity_date': project_lines[0].end_date,
                        'start_date': project_lines[0].start_date
                    })

    def send_to_executive_manger(self):
        for order in self:
            summary = 'Blanket Sale Order Is Need Your Approval !'
            group_executive_manger_approve_blanket = self.env.ref("sale_blanket_order.group_executive_manger_approve_blanket")
            users = group_executive_manger_approve_blanket.users
            self.env['user.notify'].create_activity(order, users, summary)
            order.write({"state": "waiting_executive_manger_approval"})
        return True

    def send_to_ready_to_send_quotation(self):
        for order in self:
            order.write({"state": "ready_to_send_quotation"})
            summary = 'Blanket Sale Order Is Need Your Approval !'
            group_send_quotation_by_email = self.env.ref("sale_blanket_order.group_send_quotation_by_email")
            users = group_send_quotation_by_email.users
            self.env['user.notify'].create_activity(order, users, summary)
        return True
    ...
    def send_by_email(self):
        for order in self:
            order.write({"state": "quotation_sent"})
            summary = 'Blanket Sale Order Is Need Your Approval !'
            group_confirm_order = self.env.ref("sale_blanket_order.group_confirm_order")
            users = group_confirm_order.users
            self.env['user.notify'].create_activity(order, users, summary)

            self.ensure_one()
            template_id = self.env['ir.model.data'].xmlid_to_res_id('sale_blanket_order.email_template_blanket_sale')
    ...
    @api.depends(
        "line_ids.remaining_uom_qty", "validity_date",
        # "confirmed",
    )
    ...
    def validity_daty_validation(self):
        today = fields.Date.today()
        if self.validity_date:
            if self.validity_date <= today:
                raise ValidationError('You Cant Set Date Before Today')
    ...
    def action_confirm(self):
        ...
        for order in self:
            ...
            order.write({"confirmed": True, "state": 'open', "name": name})
        return True
