...
class PoTag(models.Model):
    ...
    tag_po_ids = fields.Many2many(comodel_name="res.partner.category", string="tag", readonly=True)
...
class SoTag(models.Model):
    ...
    _inherit = 'sale.order'
    tag_so_ids = fields.Many2many(comodel_name="res.partner.category", string="tag", readonly=True)
    @api.onchange('partner_id')
